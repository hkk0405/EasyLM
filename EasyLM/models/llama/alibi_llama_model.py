...
#     bias = rearrange(np.arange(seq_len), 'j -> 1 1 j')
#     return slopes * bias
...
class FlaxLLaMAAttention(nn.Module):
    ...
    def setup(self):
        ...
        self.causal_mask = make_causal_mask(jnp.ones((1, config.max_sequence_length), dtype="bool"), dtype="bool") # [batch..., 1, len, len]
        ...
        # alibi_pos_emb
        self.alibi_bias = calc_alibi_bias(self.config.max_sequence_length, heads=self.num_heads)
    ...
    def __call__(
        ...
        # hidden_states = jnp.array(np.random.rand(1, 10, 4096))
        # attention_mask = jnp.ones((1, 10), dtype=jnp.int32)
        ...
        # alibi_pos_emb
        # alibi_bias = self.alibi_bias[:, :, :query_length] # [num_head, 1, query_length]
        # attention_bias = jnp.where(attention_mask, repeat(alibi_bias, 'h 1 j -> h i j', i=query_length), -1e10) # [batch, num_head, query_length, query_length]

        # alibi_pos_emb - paper version
        alibi_bias = self.alibi_bias[:, :query_length, :query_length] # [num_head, query_length, query_length]
        attention_bias = jnp.where(attention_mask, alibi_bias, -1e10) # [batch, num_head, query_length, query_length]
